...
class AIDiagnosisService
{
    ...
    private function buildPrompt(array $data): string
    {
        ...
        // Handle both old vehicle_info structure and new flat structure
        $make = $data['car_brand'] ?? $data['vehicle_info']['make'] ?? $data['make'] ?? 'Unknown';
        $model = $data['car_model'] ?? $data['vehicle_info']['model'] ?? $data['model'] ?? 'Unknown';
        $year = $data['car_year'] ?? $data['vehicle_info']['year'] ?? $data['year'] ?? 'Unknown';
        $mileage = $data['mileage'] ?? $data['vehicle_info']['mileage'] ?? 'Unknown';
        $engineType = $data['engine_type'] ?? $data['vehicle_info']['engine_type'] ?? 'Unknown';
        $engineSize = $data['engine_size'] ?? $data['vehicle_info']['engine_size'] ?? '';

        $prompt = "Analyze this automotive diagnosis request:\n\n";
        $prompt .= "Vehicle: {$year} {$make} {$model}\n";
        $prompt .= "Mileage: {$mileage} miles\n";
        $prompt .= "Engine: {$engineType} {$engineSize}\n\n";
        
        if (!empty($symptoms)) {
            // Ensure symptoms is an array before imploding
            ...
            $prompt .= "Reported Symptoms: " . $symptomsList . "\n\n";
        }
    ...
    {
        $symptoms = $data['symptoms'] ?? [];
        $description = strtolower($data['description'] ?? $data['problem_description'] ?? '');
