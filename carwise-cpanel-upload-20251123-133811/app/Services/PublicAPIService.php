...
namespace App\Services;
    ...
    class PublicAPIService
    {
        ...
        public function __construct()
        {
            ...
            $missingCredentials = [];
            if (!$this->ebayAppId) $missingCredentials[] = 'EBAY_APP_ID';
            if (!env('EBAY_CLIENT_SECRET')) $missingCredentials[] = 'EBAY_CLIENT_SECRET';
            if (!$this->amazonAccessKey) $missingCredentials[] = 'AMAZON_ACCESS_KEY';
            if (!$this->amazonSecretKey) $missingCredentials[] = 'AMAZON_SECRET_KEY';
            if (!$this->amazonPartnerTag) $missingCredentials[] = 'AMAZON_PARTNER_TAG';
            
            if (!empty($missingCredentials)) {
                \Log::info('PublicAPIService: Missing API credentials', ['missing' => $missingCredentials]);
            }
        }
        ...
        public function searchAmazonParts($query, $category = 'Automotive', $limit = 10)
        {
            ...
            return Cache::remember($cacheKey, 1800, function () use ($query, $category, $limit) {
                try {
                    ...
                    if (!$this->amazonAccessKey || !$this->amazonSecretKey || !$this->amazonPartnerTag) {
            ...
            return Cache::remember($cacheKey, 3600, function () {
                try {
                    ...
                    if (!$ebayAppId || !$ebayClientSecret) {
                        \Log::warning('eBay API credentials not configured');
                        return null;
                    }
                    
                    $response = Http::asForm()
                        ->withBasicAuth($ebayAppId, $ebayClientSecret)
