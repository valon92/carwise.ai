...
export function useLiveChat() {
  ...
  const connectChat = () => {
    ...
    ws.onerror = (error) => {
      console.log('Chat WebSocket error, will retry...')
      connectionStatus.value = 'error'
      
      // Switch to fallback mode after error
      setTimeout(() => {
  ...
  const loadConversationHistory = async () => {
    try {
      ...
      if (!token) {
        console.log('No auth token found, skipping conversation history load')
        return
      }
      ...
      if (data.success) {
        ...
        console.log('Conversation history loaded:', conversations.value.length, 'conversations')
      }
    } catch (error) {
