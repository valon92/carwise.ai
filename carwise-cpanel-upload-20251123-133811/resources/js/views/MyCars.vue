          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-0">
            ...
            <div class="relative lg:col-span-1 bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-800 p-8 flex items-center justify-center">
              ...
              <div class="relative w-full max-w-sm">
                <div class="relative bg-white/90 dark:bg-slate-900/90 rounded-2xl p-6 shadow-lg group">
                  <!-- Car Image -->
                  <div class="relative">
                    <img 
                    ...
                    <div class="absolute inset-0 bg-black/50 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                      <label class="cursor-pointer">
                        <input 
                          type="file" 
                          accept="image/*" 
                          @change="handleImageUpload($event, car)"
                          class="hidden"
                        />
                        <div class="bg-white/90 dark:bg-slate-800/90 rounded-full p-3 hover:bg-white dark:hover:bg-slate-700 transition-colors duration-200">
                  ...
                  <div class="mt-3 text-center">
                    <p class="text-xs text-slate-600 dark:text-slate-400">Click to upload your car photo</p>
                  </div>
                </div>
                ...
                <div class="absolute -bottom-2 -left-2 flex items-center space-x-2 bg-white/90 dark:bg-slate-900/90 rounded-full px-3 py-2 shadow-lg">
                  <div 
                    ...
                    :class="{
                      'bg-blue-500': car.maintenance_status === 'good',
            ...
            <div class="lg:col-span-1 p-8 space-y-6">
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <div v-if="car.year" class="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-2xl p-4 border border-blue-200/50 dark:border-blue-700/50">
                  <div class="text-xs font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wide mb-1">Year</div>
                  <div class="text-lg font-bold text-slate-900 dark:text-white">
                    {{ car.year }}
                  </div>
                </div>
                <div v-if="car.model" class="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-2xl p-4 border border-amber-200/50 dark:border-amber-700/50">
                  <div class="text-xs font-semibold text-amber-600 dark:text-amber-400 uppercase tracking-wide mb-1">Model</div>
                  <div class="text-lg font-bold text-slate-900 dark:text-white capitalize">
        ...
        <div class="p-8">
          <form @submit.prevent="saveCar" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label for="engine_type" class="block text-sm font-semibold text-secondary-700 dark:text-secondary-300 mb-2">
                  Engine Type
                </label>
                <select 
                  id="engine_type"
                  v-model="carForm.engine_type"
                  class="input w-full"
                >
              ...
              <div>
                <label for="engine_size" class="block text-sm font-semibold text-secondary-700 dark:text-secondary-300 mb-2">
                  Engine Size
                </label>
                <input 
                  id="engine_size"
                  v-model="carForm.engine_size"
                  type="text"
                  class="input w-full"
...
export default {
  ...
  setup() {
    ...
    const { user } = useAuth()
    const { trackCarAdded, trackEvent } = useAnalytics()
    ...
    const saveCar = async () => {
      try {
        ...
        if (response.data.success) {
          // Track car addition with analytics
          if (!editingCar.value) {
            trackCarAdded({
              ...
              engine_type: carForm.value.engine_type
            })
          }
    ...
    // Handle image upload
    const handleImageUpload = async (event, car) => {
      ...
      try {
        ...
        // Upload image
        const response = await fetch(`/api/cars/${car.id}/upload-image`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
        ...
        if (response.ok) {
          ...
          if (result.success) {
            // Update car image URL
            car.image_url = result.data.image_url
    ...
    const startDiagnosis = (car) => {
      const carData = {
        ...
        engine_type: car.engine_type || car.fuel_type,
        engine_size: car.engine_size || getEngineInfo(car)
      }
